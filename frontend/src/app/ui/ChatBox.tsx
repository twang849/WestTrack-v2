...
export function ChatBubble({ messageObject }: {messageObject: Message} ) {
  return (
    <div className={`flex ${messageObject.fromUser ? 'justify-end' : 'justify-start'} mb-2`}>
      <div className={`max-w-[35rem] px-4 py-2 rounded-2xl text-lg ${messageObject.fromUser ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-200 text-black rounded-bl-none'}`}>
        {messageObject.fromUser ? messageObject.text : <Typewriter text={messageObject.text} speed={5}></Typewriter>}
      </div>
    </div>
  );
}
